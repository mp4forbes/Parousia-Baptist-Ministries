...
import { frenchPastorMessage } from '@/lib/french-content';
    ...
    <footer className={`${bgFooter} border-t ${borderMain} pt-20 pb-8 relative`}>
      <div className={siteShellClass}>
        <div className="grid md:grid-cols-3 gap-12 mb-16">
          <div>
            ...
            <p className={`text-sm leading-relaxed mb-6 font-medium ${isLight ? 'text-slate-600' : 'text-slate-400'}`}>
              {language === 'fr_ht' ? frenchPastorMessage(settings) : settings.pastor_message_english}
            </p>
          </div>
          ...
          <div>
            <h5 className={`font-bold uppercase text-xs tracking-widest mb-4 ${textTitle}`}>{t.footerPortalsTitle}</h5>
        ...
        <div className={`pt-8 border-t ${borderDivider} flex flex-col sm:flex-row justify-between items-center text-xs font-semibold ${isLight ? 'text-slate-500' : 'text-slate-400'}`}>
          ...
          <p className="mt-2 sm:mt-0 text-slate-500">{t.footerLanguages}</p>
        </div>
      </div>
    </footer>
  );
